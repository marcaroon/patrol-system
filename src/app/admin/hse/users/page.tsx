// src/app/admin/hse/users/page.tsx
"use client";
...
export default function HSEUsersPage() {
  ...
  const load = () =>
    fetch("/api/users?division=HSE&active=false")
      .then((r) => r.json())
      .then((d) => {
        setUsers(d);
        setLoading(false);
    ...
    try {
      const res = editingId
        ? await fetch(`/api/users/${editingId}`, {
            ...
            body: JSON.stringify({ name: formName, division: "HSE" }),
          })
  ...
  const canEdit = session?.role === "SUPER_ADMIN" || session?.role === "HSE_ADMIN";
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              Kelola daftar EHS&FS Officer · {users.length} total ({activeUsers.length} aktif)
            </p>
          </div>
          {canEdit && (
            <button
        ...
        {showForm && (
          <div className="rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-4 space-y-4">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              ...
              <span className="text-emerald-300 text-sm font-medium">Divisi: EHS&FS</span>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-colors"
              >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-500">
            ...
            {canEdit && (
              <button
                onClick={openAdd}
                className="mt-4 flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold mx-auto transition-colors"
              >
...
function UserRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors">
      <div className="w-9 h-9 rounded-full bg-emerald-500/20 border border-emerald-500/20 flex items-center justify-center text-emerald-300 text-xs font-bold uppercase flex-shrink-0">
        {user.name[0]}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-xs ${user.isActive ? "text-emerald-400" : "text-gray-600"}`}>
