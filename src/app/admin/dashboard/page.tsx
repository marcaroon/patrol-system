          ...
          <div className="p-5 space-y-4">
            <div className="rounded-xl bg-red-500/10 border border-red-500/20 p-4 space-y-2">
              ...
              <ul className="text-red-400/80 text-xs space-y-1 list-disc pl-4">
                ...
                <li>Seluruh laporan EHS akan dihapus permanen</li>
                <li>Semua foto, temuan, dan data terkait akan hilang</li>
                <li>
                  Data personel, area patrol, dan akun admin{" "}
                  <strong className="text-white">tidak</strong> akan terpengaruh
                </li>
              </ul>
            </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handlePurge}
                disabled={
                  loading || typed.trim().toUpperCase() !== CONFIRM_PHRASE
                }
          ...
          <div className="p-5 space-y-4 text-center">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                Semua data laporan Security & EHS telah dihapus.
              </p>
            </div>
            <button
              onClick={() => {
...
export default function DashboardPage() {
  ...
  // Derived
  const secTrend = stats
    ? computeTrend(stats.periodData.map((d) => d.Security))
    : "flat";
  const findTrend = stats
    ? computeTrend(stats.findingsTrend.map((d) => d.findings))
    : "flat";
  const timeTrend = stats
    ? computeTrend(stats.patrolTimeStats.trend.map((d) => d.avgMinutes))
    : "flat";
  const pieData = stats
    ? [
        {
        ...
        { name: "EHS", value: stats.hseReports, color: COLORS.hse },
      ]
    : [];
      ...
      <div className="space-y-6 pb-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Analitik laporan patrol Security & EHS
            </p>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <StatCard
                ...
                sub={`${stats.reportsToday} hari ini`}
                trend={secTrend}
              />
              <StatCard
            ...
            <div>
              <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
                <SectionTitle
                  ...
                  sub="Pilih rentang waktu untuk semua grafik di bawah"
                />
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="rounded-2xl border border-purple-500/20 bg-purple-500/10 p-4">
                  ...
                  <p className="text-2xl font-bold text-purple-300">
                    {fmtMin(stats.patrolTimeStats.avg)}
                  </p>
                  <p className="text-gray-500 text-xs mt-1">
                    per laporan Security
                  </p>
                </div>
                <div className="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4">
                  ...
                  <p className="text-2xl font-bold text-blue-300">
                    {fmtMin(stats.patrolTimeStats.total)}
                  </p>
                  <p className="text-gray-500 text-xs mt-1">
                ...
                <div className="rounded-2xl border border-green-500/20 bg-green-500/10 p-4">
                  ...
                  <p className="text-2xl font-bold text-green-300">
                    {fmtMin(stats.patrolTimeStats.min)}
                  </p>
                ...
                <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4">
                  ...
                  <p className="text-2xl font-bold text-red-300">
                    {fmtMin(stats.patrolTimeStats.max)}
                  </p>
              ...
              <div className="rounded-2xl border border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-red-500/5 p-4 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">
                    {stats.totalFindingsInPeriod} temuan /{" "}
              ...
              <div className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-blue-500/5 p-4 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-cyan-300">
                    {stats.patrolTimeStats.count}
                  </p>
                  <p className="text-gray-500 text-xs mt-0.5">
                    dengan data waktu lengkap
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 card-dark p-4">
                <SectionTitle
                  icon={BarChart2}
                  title={`Laporan per Periode – ${PERIOD_LABELS[period]}`}
                />
                {stats.periodData.some((d) => d.Security + d.HSE > 0) ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart
                      data={stats.periodData}
                      barSize={period === "yearly" ? 12 : 9}
                      barGap={3}
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 card-dark p-4">
                <SectionTitle
                  icon={Timer}
                  title="Tren Rata-rata Durasi Patrol"
                  sub={`Menit per periode – ${PERIOD_LABELS[period]}`}
                />
                {stats.patrolTimeStats.trend.some((d) => d.avgMinutes > 0) ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={stats.patrolTimeStats.trend}>
                      ...
                      <Tooltip
                        {...TOOLTIP_STYLE}
                        formatter={(v: number) => [`${v} menit`, "Rata-rata"]}
                      />
                      <Area
                        type="monotone"
                        dataKey="avgMinutes"
              ...
              <div className="card-dark p-4">
                <SectionTitle
                  ...
                  sub="Persebaran waktu patrol"
                />
                {stats.patrolTimeStats.durationDist.some((d) => d.count > 0) ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart
                      data={stats.patrolTimeStats.durationDist}
                      layout="vertical"
                      barSize={14}
                    >
                      <CartesianGrid
            ...
            <div className="card-dark p-4">
              <SectionTitle
                icon={AlertTriangle}
                title="Tren Temuan vs. Kondisi Normal"
                sub={`Jumlah inspeksi per periode – ${PERIOD_LABELS[period]}`}
              />
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={stats.findingsTrend}>
                    ...
                    <Area
                      ...
                      dataKey="findings"
                      name="Ada Temuan"
                      stroke={COLORS.finding}
                      fill="url(#gFind)"
                      strokeWidth={2}
                      dot={{ fill: COLORS.finding, r: 2 }}
                    />
                    <Area
                      type="monotone"
                      dataKey="clear"
                      name="Tidak Ada Temuan"
                      stroke={COLORS.clear}
                      fill="url(#gClear)"
                      strokeWidth={2}
                      dot={{ fill: COLORS.clear, r: 2 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="card-dark p-4">
                <SectionTitle
                  icon={MapPin}
                  title="Temuan & Inspeksi per Area"
              ...
              <div className="card-dark p-4">
                <SectionTitle
                  icon={AlertTriangle}
                  title="Top Potensi Bahaya EHS"
                  sub={`Periode ${PERIOD_LABELS[period]}`}
                />
                {stats.hazardDist.length > 0 ? (
                  <ResponsiveContainer
                    width="100%"
                    height={Math.max(160, stats.hazardDist.length * 38)}
                  >
                    <BarChart
                      data={stats.hazardDist.map((d) => ({
                        ...d,
            ...
            <div className="card-dark p-4">
              <SectionTitle
                icon={Clock}
                title="Distribusi Jam Patrol Security"
                sub={`Kapan laporan paling sering masuk – ${PERIOD_LABELS[period]}`}
              />
              {stats.peakHours.some((h) => h.count > 0) ? (
                <>
                  ...
                  <p className="text-xs text-gray-600 text-center mt-1">
                    Warna merah = jam sibuk, biru = sepi
                  </p>
                </>
              ) : (
                <EmptyChart />
            ...
            {stats.byArea.length > 0 && (
              <div className="card-dark p-4">
                <SectionTitle
                  icon={TrendingUp}
                  title="Total Kunjungan per Area (Semua Waktu)"
                />
                <ResponsiveContainer
                  width="100%"
